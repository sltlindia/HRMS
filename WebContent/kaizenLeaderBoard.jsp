...
<table class="table table-bordered">
	<thead>
		<tr>
			<th colspan="2" id="first" style="background: #2471A3;color: white;">LeaderBoard-CI DashBoard</th>
			<td colspan="2"></td>
			<th colspan="2" id="third" style="background: #2471A3;color: white;">Year <%=year%></th>
		</tr>
	</thead>
	<tbody>
		<tr>
		<%for(int i = 3;i<9;i++){
			...
			KaizenAuthorityScoreBean kaiAuthorityScoreBean = allKaizenListDAO.getFinalresultOfLeaderBoardByMonthAndYear(monthBean.getMonth_id(), year1);
			List<KaizenAuthorityScoreBean> lisScoreBeans = allKaizenListDAO.listOfAuthorityScoring(year1, monthBean.getMonth_id());
			if(kaiAuthorityScoreBean != null && lisScoreBeans.size()>=3){
				int kaizen_id = kaiAuthorityScoreBean.getKaizenBean().getKaizen_id();
				KaizenBean kaizenBean = allKaizenListDAO.getDetailOfKaizenById(kaizen_id);
				 ...
				 List<KaizenProblemIdentificationBean> listOfIdentification  = allKaizenListDAO.getListOfProblemWithMonthYear(monthBean.getMonth_id(),year1);
					...
					<td height="445px;" width="200px;">
						...
						<div class="row">
							...
							<img alt="<%=kaizenBean.getEmployeeBean().getFirstname()+" "+kaizenBean.getEmployeeBean().getLastname()%>" src="FileServlet?path=D:\hrms\upload\profilePic\<%=company_name%>\<%=emp_code%>.bmp" height="60px;" width="60px;" style="border: solid 1px;">
								...
								(<%=kaizenBean.getEmployeeBean().getDepartmentBean().getDepartment_name()%>)
								</p>
		...
		<%for(int i = 0;i<3;i++){
			MonthBean monthBean = listOfMonth.get(i);
			KaizenAuthorityScoreBean kaiAuthorityScoreBean = allKaizenListDAO.getFinalresultOfLeaderBoardByMonthAndYear(monthBean.getMonth_id(), year2);
			List<KaizenAuthorityScoreBean> lisScoreBeans = allKaizenListDAO.listOfAuthorityScoring(year2, monthBean.getMonth_id());
			if(kaiAuthorityScoreBean != null && lisScoreBeans.size()>=3){
				int kaizen_id = kaiAuthorityScoreBean.getKaizenBean().getKaizen_id();
				KaizenBean kaizenBean = allKaizenListDAO.getDetailOfKaizenById(kaizen_id);
				 List<KaizenAfterUploadAttachmentBean> listOfAfter = allKaizenListDAO.getListOfAfterUploadAttachment(kaizen_id);
