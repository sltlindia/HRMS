...
List<KaizenBean> lisOfMainKaizen = allKaizenListDAO.getListOfAllCompletedKaizenBymanagerId(manager_id,year,month);
KaizenBillboardResultBean kaizenBillboardResultBean = allKaizenListDAO.getDetailOfKaizenBillBoard(employee_id, year, month);
...
<%  for(int i =0;i<lisOfMainKaizen.size();i++){
      			KaizenBean kaizenBean = lisOfMainKaizen.get(i);
      			...
      			if(kaizenManagerScoreBean != null){
      				 qua = kaizenManagerScoreBean.getQuality();
          			 cost = kaizenManagerScoreBean.getCost();
          			 saf = kaizenManagerScoreBean.getSafety();
          			 prod = kaizenManagerScoreBean.getProductivity();
          			 del = kaizenManagerScoreBean.getDelivery();
          			 hor = kaizenManagerScoreBean.getHorizontal_deployment();
          			 ave = kaizenManagerScoreBean.getTotal();
      			}
...
<div class="panel panel-default" id="">
      ...
      <font style="font-weight: bold;" size="2px"><a style="cursor: pointer;" onclick="window.open('kaizenView.jsp?kaizen_id=<%=kaizen_id%>','_blank');"><%=kaizenBean.getKaizen_name() %></a></font>
      </div>
      <ul class="collapsible popout" data-collapsible="accordion">
        
          
      		 <li>
            <div class="collapsible-header active" onclick="imageSwap('img<%=kaizen_id%>');"><i class="material-icons" id="img<%=kaizen_id%>">chevron_right</i><b><%=kaizenBean.getKaizen_name() %></b>
