...
/**
 * Servlet implementation class UnplannedTaskManagerUpdateAfterRejectServlet
 */
public class UnplannedTaskManagerUpdateAfterRejectServlet extends HttpServlet {
					...
					if (item.isFormField()) {
						...
						if (fieldName.equalsIgnoreCase("percentage_completed")) {
							...
							if (workedHour != 0) {
								...
								List<Object[]> listOftimesheet = allListDAO.duplicateTimeSheetList(employee_id, date,
								...
								for (Object[] aRow : listOftimesheet) {
									...
									if (sheetBean.getTimesheet_master_id() != 0) {
										...
										request.getRequestDispatcher("newUnplannedValidationManager").forward(request,
												response);
										return;
									}
								}

								TimeSheetBean timeSheetBean = new TimeSheetBean(employeeBean, projectMasterBean,
										taskMasterBean, clientMasterBean, dayBean, date, workedHour, employee_comment,
										task_time_status, approval_status, percentage_completed,task_desc);
								boolean result = allInsertDAO.timeSheetInsert(timeSheetBean);
									AllUpdateDAO allUpdateDAO = new AllUpdateDAO();
									boolean result1 = allUpdateDAO.timeSheetEmployeeCommentUpdate(employee_id, date,
											employee_comment);
