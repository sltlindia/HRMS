...
public class AppraisalSection1InsertServlet extends HttpServlet {
			...
			for (int i = 0; i < items.size(); i++) {
				...
				if (item.isFormField()) {
					...
					if (fieldName.equalsIgnoreCase("attendanceGrade")) {
						...
						AppraisalLeaveBean appraisalLeaveBean  = new AppraisalLeaveBean(leave, lateComing, appraisalBean, appraisalGradeBean);
						boolean result = allInsertDAO.appraisalLeaveInsert(appraisalLeaveBean);
					...
					if (fieldName.equalsIgnoreCase("kaizenGrade")) {
						...
						AppraisalKizenBean appraisalKizenBean = new AppraisalKizenBean(totalKaizen, approvedKaizen, remarkKizen, appraisalBean, appraisalGradeBean);
						boolean result = allInsertDAO.appraisalKizenInsert(appraisalKizenBean);
					}
					...
					if (fieldName.equalsIgnoreCase("qGrade")) {
						...
						Appraisal5sBean appraisal5sBean = new Appraisal5sBean(qScore, qRemarks, appraisalBean, appraisalGradeBean, quterlyMonthBean);
						boolean result = allInsertDAO.appraisal5sInsert(appraisal5sBean);
					...
					if (fieldName.equalsIgnoreCase("tTrainingRemark")) {
						...
						System.out.println("tTrainingRemark:"+tTrainingRemark);
							...
							AppraisalTrainingBean appraisalTrainingBean = new AppraisalTrainingBean(tTopic, tFaculty, tDate, tHrs, tTrainingRemark, appraisalBean);
							boolean result = allInsertDAO.appraisalTrainingInsert(appraisalTrainingBean);
						}	
					}
					...
					if (fieldName.equalsIgnoreCase("redirect1")) {
						request.setAttribute("appraisal_id", appraisal_id);
						request.getRequestDispatcher("whiteCollarAppraisalManagerSection3.jsp").forward(request, response);
					}		
