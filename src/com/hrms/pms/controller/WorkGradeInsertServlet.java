...
public class WorkGradeInsertServlet extends HttpServlet {
			...
			for (int i = 0; i < items.size(); i++) {
				...
				if (item.isFormField()) {
					...
					if (fieldName.equalsIgnoreCase("grade_id")) {
						...
						WorkGradeBean workGradeBean = new WorkGradeBean(appraisalBean, workBehaviourBean, appraisalGradeBean);
						boolean result = allInsertDAO.workGradeInsert(workGradeBean);
					}
