...
/**
 * Servlet implementation class FamilyDetailUpdateServlet
 */
public class FamilyDetailUpdateServlet extends HttpServlet {
				...
				try {
					...
					for (int i = 0; i < items.size(); i++) {
						...
						if (item.isFormField()) {
							...
							if (fieldName.equalsIgnoreCase("family_detail_id")) {
								String value = fieldValue;
								family_detail_id = Integer.parseInt(value);
								System.out.println("family_detail_id:"+family_detail_id);
							}
							...
							if (fieldName.equalsIgnoreCase("update")) {
								...
								FamilyDetailBean familyDetailBean = new FamilyDetailBean(family_detail_id,name, relation, dob, occupation, contact_number, adhaar_number, employeeBean, familyRelationBean);
								boolean result = allUpdateDAO.familyDetailUpdate(familyDetailBean);  //Update Method
