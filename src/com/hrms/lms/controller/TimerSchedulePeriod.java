...
public class TimerSchedulePeriod extends TimerTask {
	...
	@Override
	public void run() {
		System.out.println("timer working : "+date.toString());

		List<LeaveBean> listOfLeave = allLMSListDAO.getLeave();
		List<LeaveCOBean> listOfCo = allLMSListDAO.getCO();
		List<LeaveODBean> listOfOD = allLMSListDAO.getOD();
		List<LeaveCancelRequestBean> listOfCancelLeave = allLMSListDAO.getListOfCancelLeave();
		...
		for(LeaveBean leaveBean : listOfLeave){
			...
			new Thread(new Runnable() {
			    ...
			    public void run() {
			    	...
			    	Mailer.send(to, sub, leavefrom, leaveto, days, emp_code,mobileNo,alert,leaveId,name);
		...
		for(LeaveCOBean leaveCOBean : listOfCo){
			...
			new Thread(new Runnable() {
			    ...
			    public void run() {
			    	...
			    	Mailer.coPendingSend(to, sub, coDate, totalHours, coCount, emp_code, name, purpose,co_id);
