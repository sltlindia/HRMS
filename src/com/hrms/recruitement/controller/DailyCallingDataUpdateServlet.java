...
public class DailyCallingDataUpdateServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int daily_calling_data_id = Integer.parseInt(request.getParameter("daily"));
		int employee_master_id = Integer.parseInt(request.getParameter("employee_id"));

		AllUpdateDAO allUpdateDAO = new AllUpdateDAO();
		boolean result = allUpdateDAO.dailyCallingDataUpdate(daily_calling_data_id, vacancy_id, 
				source_of_resume_id, department_id, employee_master_id, candidate_name, current_location, 
				mobile_number, email_id, current_designation, total_experience, highest_qualification, 
				current_CTC, expected_CTC, notice_period, status, remarks, offered_gross, offered_CTC, 
				joining_date, submission_date);
		
		
		if(result == true)
		{
			response.sendRedirect("dailyCallingDataList.jsp");
		}
	}
