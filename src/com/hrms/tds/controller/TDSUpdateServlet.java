...
public class TDSUpdateServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public TDSUpdateServlet() {
        super();
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(user!=null)
		{
			...
			tds_id = Integer.parseInt(request.getParameter("tdsId"));
			...
			try{
				...
				String status_change = "updated";
				TDSBean tdsBean = new TDSBean();
				tdsBean.setTds_id(tds_id);
				
				TDSBean BeanId = tdsListDAO.detailsOfTDS(tds_id);
				TDSTotalABean aBeanId = tdsListDAO.detailsOfTDSTotal_A(tds_id);
				TDSTotalBBean bBeanId = tdsListDAO.detailsOfTDSTotal_B(tds_id);
				TDSTotalCBean cBeanId = tdsListDAO.detailsOfTDSTotal_C(tds_id);
				TDSTotalDBean dBeanId = tdsListDAO.detailsOfTDSTotal_D(tds_id);
				TDSTotalEBean eBeanId = tdsListDAO.detailsOfTDSTotal_E(tds_id);
				TDSTotalFBean fBeanId = tdsListDAO.detailsOfTDSTotal_F(tds_id);
				...
				boolean resultTotalA = tdsUpdateDAO.tdsTotalAInsert(aBean);
				boolean resultTotalB = tdsUpdateDAO.tdsTotalBInsert(bBean);
				boolean resultTotalC = tdsUpdateDAO.tdsTotalCInsert(cBean);
				boolean resultTotalD = tdsUpdateDAO.tdsTotalDInsert(dBean);
				boolean resultTotalE = tdsUpdateDAO.tdsTotalEInsert(eBean);
				boolean resultTotalF = tdsUpdateDAO.tdsTotalFInsert(fBean);
				...
				if(taxList.size() != 0)
				{
					...
					TDSTaxBean taxBean = new TDSTaxBean(taxBeanId.getTds_tax_id(), end_year_int, tax_payable, tdsBean, employeeBean, monthBean);
				...
				boolean result1 = tdsUpdateDAO1.tdsStatusUpdateHrSide(EMP_MASTER_ID, status_change);
