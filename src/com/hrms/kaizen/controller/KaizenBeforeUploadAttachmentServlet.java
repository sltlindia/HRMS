...
public class KaizenBeforeUploadAttachmentServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(user!=null){
		String filePath= request.getServletContext().getInitParameter("kaizen-upload-before");
				...
				try {
					...
					for (int i = 0; i < items.size(); i++) {
						...
						if (item.isFormField()) {
							...
							if (fieldName.equalsIgnoreCase("firstInsert")) {
								firstInsert = fieldValue;
								System.out.println("firstInsert:"+firstInsert);
							}
							...
							try {
								...
								if (fileName.isEmpty()) {
									...
									KaizenBeforeUploadAttachmentBean kaizenBeforeUploadAttachmentBean= new KaizenBeforeUploadAttachmentBean(attachment,kaizenBean);
									boolean result = allKaizenInsertDAO.kaizenPhotoUploadBeforeInsert(kaizenBeforeUploadAttachmentBean);
